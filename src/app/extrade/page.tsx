...
export default function Extrade() {
  ...
  useEffect(() => {
    const savedDeposits = localStorage.getItem('extrade-deposits');
    if (savedDeposits) {
      setDeposits(JSON.parse(savedDeposits));
    }
    const savedConversions = localStorage.getItem('extrade-conversions');
    if (savedConversions) {
  ...
  useEffect(() => {
    localStorage.setItem('extrade-deposits', JSON.stringify(deposits));
  }, [deposits]);

  useEffect(() => {
    localStorage.setItem('extrade-conversions', JSON.stringify(conversions));
  }, [conversions]);
